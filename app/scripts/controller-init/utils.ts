 ...
 * @param options.existingServices - All required services that have already been initialized.
...
export function initControllers({
  ...
  const partialServicesByName = existingServices.reduce<Partial<ServiceByName>>(
    (acc, service) => {
      // @ts-expect-error: Union too complex.
  ...
  const getService = <Name extends ServiceName>(
    name: Name,
  ): ServiceByName[Name] =>
    getServiceOrThrow(partialServicesByName as ServiceByName, name);
...
function getServiceOrThrow<Name extends ServiceName>(
  servicesByName: ServiceByName,
  name: Name,
): ServiceByName[Name] {
  const service = servicesByName[name];

  if (!service) {
    throw new Error(`Service requested before it was initialized: ${name}`);
  }
