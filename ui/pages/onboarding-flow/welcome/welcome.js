...
export default function OnboardingWelcome() {
  ...
  const onSocialLoginClick = async (loginType, loginOption) => {
    setIsLoggingIn(true);
    try {
      setNewAccountCreationInProgress(true);
      dispatch(setFirstTimeFlowType(FirstTimeFlowType.social));

      const isNewUser = await dispatch(startOAuthLogin(loginType));
  ...
  const handleLogin = async (loginType, loginOption) => {
    if (loginType === LOGIN_TYPE.SRP) {
      ...
      await onSocialLoginClick(loginType, loginOption);
    }
  };
